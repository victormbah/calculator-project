...
def handle_button_click (clicked_botton_text):
    current_text = result_var.get()
    
    if clicked_botton_text == "=":
        try:
            # Replace custom Symblos with python operators
            expression = current_text.replace("÷", "/").replace("×", "*")
            result = eval(expression)
            
            # check if thebresult is a whole number 
    ...
    elif clicked_botton_text == "C":
        result_var.set("")
    elif clicked_botton_text == "%":
        # convert the curent number to a decimal by dividing it by 100
        try:
        ...
        result_var.set(current_text + clicked_botton_text)

 # Create the main widow 


root = tk.Tk()
root.title("Calculator")

# Enter widget to display the result with larger font size 
